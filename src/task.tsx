...
export function TaskComponent(props : {header : string, content : string, priority : "high" | "medium" | "low", deadline : string}) {
                        ...
                        <div className="absolute cursor-default w-[100vw] h-[100vh] top-[0%] left-[0%] flex items-center justify-center bg-black/66">
                            <div className="flex flex-col w-fit max-w-[60vw] h-fit gap-[clamp(5px,4vh,40px)] px-[clamp(10px,2vw,30px)] py-[clamp(10px,3vh,30px)] self-center justify-self-center bg-plate-task rounded-[12px]">
                                ...
                                <div className="flex flex-col gap-[clamp(5px,4vh,40px)] h-[100%] justify-between">
                                    <h2 className="font-semibold text-task-header px-[clamp(5px,1vw,20px)] text-[clamp(0.5rem,2vw,2rem)]">{props.header}</h2>
                                    <p className="font-medium text-[clamp(0.5rem,1.5vw,1.5rem)] text-task-content max-h-[40vh] overflow-y-auto">{props.content}</p>
                                    <div className="flex flex-row justify-between">
                                        <p className={`w-min text-priority-${props.priority} font-medium text-[clamp(0.5rem,1vw,1rem)]`}>{`Приориет: ${props.priority}`}</p>
                                        <p className="w-min font-medium text-[clamp(0.5rem,1vw,1rem)] text-task-content">{`Дедлайн: ${props.deadline}`}</p>
                                    </div>
